...
<template>
    <div v-if="props.open" class="fixed inset-0 z-50 flex items-center justify-center" @click="emit('close')">
        <div class="absolute inset-0 bg-black/40" />

        <div class="bg-black relative w-full max-w-md px-10 py-8 text-white" @click.stop>
            <h2 class="text-2xl font-medium" style="font-size: 30px;">Registrasi</h2>

            <form @submit.prevent="submit" class="mt-4 space-y-3">
                <div>
                    <input v-model="form.nama" type="text" autocomplete="name" placeholder="Nama"
                        class="bg-white text-black w-full px-3 py-2 text-sm" required />
                ...
                <div>
                    <input v-model="form.whatsapp" type="tel" inputmode="tel" autocomplete="tel"
                        placeholder="Nomor WhatsApp" class="bg-white text-black w-full px-3 py-2 text-sm" required />
                ...
                <div>
                    <div class="relative">
                        <input v-model="form.password" :type="showPassword ? 'text' : 'password'"
                            ...
                            class="bg-white text-black w-full px-3 py-2 pr-10 text-sm" required />
                        <button type="button" class="text-white-disable absolute right-2 top-1/2 -translate-y-1/2"
                ...
                <Button type="submit" variant="light" class="mt-8 w-full py-2 text-sm" :disabled="form.processing">
                    Registrasi
                </Button>
            </form>
        </div>
