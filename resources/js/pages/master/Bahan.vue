...
const editingItem = ref<BahanData | null>(null);
...
function openEdit(item: BahanData) {
    editingItem.value = item; form.clearErrors();
    form.id_kategori = String(item.id_kategori);
    form.id_warna = item.id_warna ? String(item.id_warna) : '';
    form.id_ukuran = String(item.id_ukuran);
    form.nama = item.nama;
    form.stok = item.stok;
...
function submitForm() {
    if (editingItem.value) {
        form.put(`/master/bahan/${editingItem.value.id}`, { onSuccess: () => closeModal() });
    } else {
        form.post('/master/bahan', { onSuccess: () => closeModal() });
    }
}

function hapusItem(id: number) {
    if (confirm('Yakin ingin menghapus bahan ini?')) {
        router.delete(`/master/bahan/${id}`, { preserveScroll: true });
    }
}
...
const items = computed(() => props.bahan?.data ?? []);
const paginationLinks = computed(() => props.bahan?.links ?? []);
...
<template>
    ...
    <div class="bg-white min-h-screen flex">
        ...
        <main class="flex-1 p-8">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-black text-xl font-medium">Bahan</h1>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-if="items.length === 0">
                            <td colspan="11" class="px-3 py-6 text-center text-black/50">Belum ada bahan.</td>
                        </tr>
                        <tr v-for="(item, idx) in items" :key="item.id"
                            class="border-t border-black/5 cursor-pointer hover:bg-black/[0.02]"
                            @click="openEdit(item)">
                            <td class="px-3 py-3 text-black">{{ idx + 1 }}</td>
                            <td class="px-3 py-3 text-black font-medium uppercase">{{ item.nama }}</td>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            <h2 class="text-black text-lg font-medium mb-4">{{ editingItem ? 'Edit Bahan' : 'Tambah Bahan' }}</h2>
